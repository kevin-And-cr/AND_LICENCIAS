...
def list_licencias(db: Session, cliente_id: int | None = None) -> list:
    rows = db.execute(
        text("EXEC dbo.usp_licencias_listar @cliente_id = :cliente_id"),
...
def get_licencia(db: Session, licencia_id: int) -> dict:
    row = db.execute(
        text("EXEC dbo.usp_licencias_obtener @id = :id"),
        {"id": licencia_id},
    ).fetchone()
    if not row:
...
def get_detalle_licencia(db: Session, licencia_id: int) -> dict:
    ...
    mods = db.execute(
        text("EXEC dbo.usp_licencias_modulos_detalle @licencia_id = :licencia_id"),
    ...
    for mod in mods:
        ...
        comps = db.execute(
            text("EXEC dbo.usp_licencias_companias_modulo @licencia_modulo_id = :licencia_modulo_id"),
            {"licencia_modulo_id": mod_dict["id"]},
        ).fetchall()
...
def create_licencia(db: Session, data: LicenciaCreate, usuario_id: int) -> dict:
    # Validación de cliente y número de licencia se conserva en Python, pero se canaliza hacia SP
    # para la parte de creación y el resultado se vuelve a hidratar por detalle.
    cl = db.execute(
        text("EXEC dbo.usp_clientes_estado @id = :id"),
        {"id": data.cliente_id},
    ).fetchone()
    if not cl:
    ...
    conflict = db.execute(
        text("EXEC dbo.usp_licencias_conflicto @numero_licencia = :numero_licencia, @id = NULL"),
    ...
    """), {
        ...
        "cantidad_maxima_usuarios": data.cantidad_maxima_usuarios,
        "permite_jobs_segundo_plano": 1 if data.permite_jobs_segundo_plano else 0,
        "permite_api": 1 if data.permite_api else 0,
...
def update_licencia(db: Session, licencia_id: int, data: LicenciaUpdate) -> dict:
    ...
    conflict = db.execute(
        text("EXEC dbo.usp_licencias_conflicto @numero_licencia = :numero_licencia, @id = :id"),
        {"numero_licencia": data.numero_licencia, "id": licencia_id},
    ).fetchone()
    if conflict:
        raise HTTPException(status_code=400, detail="El número de licencia ya existe")

    row = db.execute(text("""
        EXEC dbo.usp_licencias_actualizar
            @id = :id,
            @numero_licencia = :numero_licencia,
            @tipo_licencia = :tipo_licencia,
            @ambiente = :ambiente,
...
def cambiar_estado(db: Session, licencia_id: int, nuevo_estado: str) -> dict:
    ...
    get_licencia(db, licencia_id)
    row = db.execute(text("EXEC dbo.usp_licencias_cambiar_estado @id = :id, @estado = :estado"), {"id": licencia_id, "estado": nuevo_estado}).fetchone()
    db.commit()
    if not row:
        raise HTTPException(status_code=404, detail="Licencia no encontrada")
    return get_detalle_licencia(db, licencia_id)
...
def agregar_modulo(db: Session, licencia_id: int, data: AgregarModuloRequest) -> dict:
    ...
    mod = db.execute(
        text("EXEC dbo.usp_modulos_obtener_por_id @id = :id"),
        {"id": data.modulo_id},
    ).fetchone()
    if not mod:
    ...
    dup = db.execute(
        text("EXEC dbo.usp_licencias_modulo_existe @licencia_id = :licencia_id, @modulo_id = :modulo_id"),
        {"licencia_id": licencia_id, "modulo_id": data.modulo_id},
    ).fetchone()
    if dup:
        raise HTTPException(status_code=400, detail="El módulo ya está asignado a esta licencia")

    companias_activas = db.execute(
        text("EXEC dbo.usp_companias_activas_cliente @cliente_id = :cliente_id"),
    ...
    row = db.execute(text("""
        EXEC dbo.usp_licencias_agregar_modulo
            @licencia_id = :licencia_id,
            @modulo_id = :modulo_id,
            @cantidad_maxima_companias = :cantidad_maxima_companias
    """), {
        "licencia_id": licencia_id,
        "modulo_id": data.modulo_id,
        "cantidad_maxima_companias": data.cantidad_maxima_companias,
    }).fetchone()
    db.commit()
    if not row:
        raise HTTPException(status_code=400, detail="No se pudo asignar el módulo")
    return get_detalle_licencia(db, licencia_id)
...
def quitar_modulo(db: Session, licencia_id: int, licencia_modulo_id: int) -> None:
    ...
    lm = db.execute(
        text("EXEC dbo.usp_licencias_modulo_existe_id @licencia_modulo_id = :licencia_modulo_id, @licencia_id = :licencia_id"),
        {"licencia_modulo_id": licencia_modulo_id, "licencia_id": licencia_id},
    ).fetchone()
    if not lm:
        raise HTTPException(status_code=404, detail="Módulo no encontrado en esta licencia")

    db.execute(text("EXEC dbo.usp_licencias_quitar_modulo @licencia_modulo_id = :licencia_modulo_id"), {
        "licencia_modulo_id": licencia_modulo_id,
    })
    db.commit()
